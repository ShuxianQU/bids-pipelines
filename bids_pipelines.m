...
%% just for re-run the final part
rerunProcessing= true;

if rerunProcessing 
    disp('=> Re-running the processing...')

    subj = {'054' '037' '050' '039'};
    task = {'restNomocoEO'};

    for snd = 1:length(subj)
        for tnd = 1:length(task)
            ...
            % disp('=> Starting xcp-d...')
            % mycmd= ['apptainer run --cleanenv ', parentDir_containers,filesep,'xcp_d-0.10.1.simg ', path2bidsNewProj,filesep,'derivatives_dummyRemoved ',...
            %         path2bidsNewProj,filesep,'derivatives_dummyRemoved/xcp_d participant --participant-label ',subID,' --mode linc -t ',taskID,...
            %         ' -w ',path2bids,'/workdir_xcp-d --motion-filter-type lp --band-stop-min 6',...
            %         ' --warp-surfaces-native2std --bids-filter-file ',path2bidsNewProj,filesep, 'derivatives_dummyRemoved/xcp_d-bidsFilterFile.json'];
            % mycmd= ['apptainer run --cleanenv ', parentDir_containers,filesep,'xcp_d-0.10.1.simg ', path2bidsNewProj,filesep,'derivatives_dummyRemoved ',...
            %         path2bidsNewProj,filesep,'derivatives_dummyRemoved/xcp_d participant --participant-label ',subID,' --mode linc -t ',taskID,...
            %         ' -w ',path2bids,'/workdir_xcp-d --dummy-scans 5',...
            %         ' --warp-surfaces-native2std --bids-filter-file ',path2bidsNewProj,filesep, 'derivatives_dummyRemoved/xcp_d-bidsFilterFile.json'];
            % 
            % system(mycmd);    
            % disp('-> xcp-d is done...')
